...
| | |
| --- | --- |
| Slug | `wall-slide-mini-band` |
| Nom (FR / EN) | Wall slide avec mini-bande / Wall Slide with Mini Band |
| Catégorie | Tronc |
| Groupe musculaire principal | Épaules |
| Muscles primaires | Trapèzes inférieurs, Rotateurs externes |
| Muscles secondaires | Rhomboïdes, Deltoïde postérieur, Coiffe des rotateurs |
| Matériel | Mini-bande |
...
- [x] Bande visible et tendue sur les 2 positions, mur conservé.
- [x] Données complètes dans `exercise.json`.
- [x] Enregistré dans le registre (`static/exercises/index.json`).
- [x] Anti-doublon : aucun wall slide (avec ou sans bande) dans la Exercise DB
      (les « wall » existants = push-up/étirements, mouvements différents).
